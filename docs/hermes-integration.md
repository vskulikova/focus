...
Новый утиль `scripts/read_current_phase.py` (живёт в этом репо или симметрично в `life-planner/scripts/hermes/`, читая отсюда):
- парсит `data/current-phase.md` секцию «Сейчас»
- возвращает dict `{kcal: 1900, protein_g_min: 100, phase: "Неделя 1", since: "2026-05-11"}`
- если файл отсутствует — возвращает `None`, Hermes молча пропускает nutrition-блок
